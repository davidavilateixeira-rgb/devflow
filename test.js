  ...
  document.getElementById("view-projeto").innerHTML = `
        ...
        ${flx.map((et,i)=>{
          ...
          const linhaEtapa = `${tituloGrupo}<div class="chk-linha ${atual?'!bg-orange-50':''}">
            ...
            <div class="flex-1">
              ...
              ${prazoEtapa?`<div class="text-[10.5px] mt-1 rounded-md p-1.5 leading-snug font-medium" style="background:#E8F1FB;color:#0F5FA8">🎯 Prazo: ${fmtL(prazoEtapa)}</div>`:""}
    ...
    <div class="grid md:grid-cols-3 gap-4 mt-4">
      ...
      <div class="card p-5 surgir md:col-span-3">
        ...
        ${(()=>{
          const fo = p.fo050 || { celulas:[] };
          if(!fo.celulas.length && !fo.maquina && !fo.tempoUsinagem && !fo.numOP) return '<div class="text-[12px]" style="color:var(--texto-2)">Nenhum dado FO050 registrado.</div>';
          return `<div class="grid md:grid-cols-4 gap-4 bg-gray-50 p-4 rounded-lg border border-gray-200">
            <div>
            ...
            <div>
              ...
              <div class="text-[13px] mt-1 font-medium">${esc(fo.maquina)||"—"}</div>
            </div>
            <div class="md:col-span-2 grid grid-cols-2 gap-4">
              <div>
                <div class="text-[11px] font-bold text-gray-500 uppercase tracking-wide">Tempos (s)</div>
                <div class="text-[13px] mt-1">Usinagem: <b>${fo.tempoUsinagem||"—"}</b></div>
                <div class="text-[13px]">Troca: <b>${fo.tempoTroca||"—"}</b></div>
                <div class="text-[13px] mt-1 pt-1 border-t border-gray-200">Total: <b class="text-blue-700">${fo.tempoUsinagem||fo.tempoTroca ? (Number(fo.tempoUsinagem||0)+Number(fo.tempoTroca||0)).toFixed(1) : "—"}</b></div>
              </div>
              <div>
                <div class="text-[11px] font-bold text-gray-500 uppercase tracking-wide">Execução</div>
                <div class="text-[13px] mt-1">Data: <b>${fo.dataExecucao?fmtL(fo.dataExecucao):"—"}</b></div>
                <div class="text-[13px]">OP: <b>${esc(fo.numOP)||"—"}</b></div>
              </div>
            </div>
          </div>`;
        })()}
...
function reabrirEtapa(id, i){
  ...
  // Limpa os dados vinculados às etapas que estão sendo desfeitas
  const etapasDesfeitas = flx.slice(alvo);
  if(etapasDesfeitas.includes("Análise")) {
    // Ao desmarcar Análise, limpa a definição de fixação e datas dependentes
    p.prazoProjetoFixacao = "";
    ...
    // Ao desmarcar Projeto da Fixação, limpa ferramentas e prazos de compra/fornecedor
    p.ferramentas = [];
    p.prazoFornecedor = "";
    p.numSC = "";
    p.numOC = "";
...
function abrirModalFO050(id, requiredMode, callback){
    ...
    return procDraft.map((pr, i) => `
      <div class="mb-4 p-4 rounded bg-white border border-gray-200 shadow-sm relative">
        ...
        ${requiredMode !== 'maquina' ? `
          ...
          <div class="text-right text-[12px] font-bold text-blue-800">Parcial: ${pr.tempoUsinagem||pr.tempoTroca ? (Number(pr.tempoUsinagem||0)+Number(pr.tempoTroca||0)).toFixed(1) : "0.0"} s</div>
        ` : ''}
      </div>
    `).join("");
  };
  ...
  window.__updateProc = (i, field, val) => {
    ...
    if(field === 'tempoUsinagem' || field === 'tempoTroca') {
      document.getElementById("listaProcessosFO050").innerHTML = window.__renderProcessosFO050(); // re-render for total
  ...
  window.__calcTotalGeralFO050 = () => {
    ...
    if(!el) return;
    const total = procDraft.reduce((acc, pr) => acc + (Number(pr.tempoUsinagem)||0) + (Number(pr.tempoTroca)||0), 0);
    el.innerText = total.toFixed(1) + " s";
  };
